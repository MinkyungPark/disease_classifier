...
df = pd.read_csv('android/total.csv', encoding='utf-8')
...
for epoch in range(5):
    ...
    history = model.fit(X_train, y_train, batch_size=32, epochs=300, callbacks=[early_stop], verbose=1, shuffle=True) # validation_split=0.25

    print('ACC : ', history.history['acc'][-1])
    print('LOSS : ', history.history['loss'][-1])
    # print('테스트 정확도 : %.4f' % (model.evaluate(X_test,y_test)[1]))

    model.save('android_cnn_'+ str(history.history['acc'][-1]) + '_' + str(epoch) + '.h5')
